...
drop policy if exists "Allow editors to update own systems" on systems;
drop policy if exists "Allow editors to view own systems" on systems;
drop policy if exists "Allow editors to delete own draft systems" on systems;
drop policy if exists "Allow authenticated users view own systems" on systems;
drop policy if exists "Allow admin full access" on systems;

-- Create new policies with correct permissions
create policy "Allow public read published systems"
  on systems for select
  to anon, authenticated
  using (status = 'published');

create policy "Allow authenticated view own systems"
  on systems for select
  to authenticated
  using (
    auth.uid() = created_by
